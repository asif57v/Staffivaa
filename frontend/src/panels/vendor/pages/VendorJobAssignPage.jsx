...
export function VendorJobAssignPage() {
  ...
  const isReadyToSubmit = requestedSkills.every(skill => 
                ...
                {eligibleCrew.map(worker => {
                  ...
                  return (
                    <li key={worker._id}>
                      ...
                      >
                        <AppSurface className={`flex items-center gap-3 p-3 transition-colors ${isSelectedForThis ? 'border-brand bg-brand/5 ring-1 ring-brand' : 'hover:border-slate-300'}`}>
                          {isSelectedForThis ? (
