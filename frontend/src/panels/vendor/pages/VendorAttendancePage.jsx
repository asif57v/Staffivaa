...
import { Clock, Users, CheckCircle, Play, AlertCircle, Building2, Calendar } from 'lucide-react'
...
export function VendorAttendancePage() {
  ...
  projects.forEach(p => {
    ...
    workingNowCount += p.workingNow || 0
    completedCount += p.completedToday || 0
  })
  ...
  return (
    <div className="space-y-6 pb-8">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Crew Dashboard</p>
          <h2 className="text-2xl font-extrabold text-slate-900">Attendance Monitoring</h2>
          <p className="mt-1 text-sm text-slate-600">Real-time attendance logs for workers assigned to your jobs.</p>
        </div>
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <AppSurface className="p-4 flex flex-col justify-center items-center text-center border-b-4 border-b-slate-400">
              ...
              <p className="text-[10px] font-bold uppercase text-slate-500 mt-1">Total Assigned</p>
            </AppSurface>
            <AppSurface className="p-4 flex flex-col justify-center items-center text-center border-b-4 border-b-brand">
              ...
              <p className="text-[10px] font-bold uppercase text-slate-500 mt-1">Present (in range)</p>
            </AppSurface>
            <AppSurface className="p-4 flex flex-col justify-center items-center text-center border-b-4 border-b-sky-500">
              <Play className="h-6 w-6 text-sky-500 mb-2" />
              <p className="text-2xl font-black text-slate-900">{workingNowCount}</p>
              <p className="text-[10px] font-bold uppercase text-slate-500 mt-1">Working Now</p>
            </AppSurface>
            <AppSurface className="p-4 flex flex-col justify-center items-center text-center border-b-4 border-b-emerald-500">
              <CheckCircle className="h-6 w-6 text-emerald-500 mb-2" />
              <p className="text-2xl font-black text-slate-900">{completedCount}</p>
              <p className="text-[10px] font-bold uppercase text-slate-500 mt-1">Completed</p>
            </AppSurface>
            <AppSurface className="p-4 flex flex-col justify-center items-center text-center border-b-4 border-b-rose-500">
              ...
              <p className="text-[10px] font-bold uppercase text-slate-500 mt-1">Absent (in range)</p>
            </AppSurface>
          </div>
          ...
          {projects.length === 0 ? (
            <AppEmptyState icon={Clock} title="No active projects" subtitle="There are no active assignments or attendance records to display." />
          ) : (
                  ...
                  <div className="bg-slate-50 p-4 border-b border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-4 mt-2">
                        ...
                        <p className="text-xs font-semibold text-emerald-600">Present (in range): {project.present}</p>
                        <p className="text-xs font-semibold text-rose-600">Absent (in range): {project.absent}</p>
                      </div>
                    </div>
                  </div>
