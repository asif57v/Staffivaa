...
export function AdminSponsoredAdsPage() {
  ...
  const handleAddSubmit = async (e) => {
    e.preventDefault()
    try {
      const res = await apiClient.post('/admin/marketing/ads', formData)
      if (res.data.success) {
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
            ...
            >
              <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                <h3 className="font-bold text-lg text-slate-800">Add Sponsored Ad</h3>
              ...
              <form onSubmit={handleAddSubmit} className="p-5 space-y-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">Banner Image URL</label>
                  <input type="url" required value={formData.banner} onChange={e => setFormData({...formData, banner: e.target.value})} className="w-full border border-slate-200 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="https://..." />
                </div>
                ...
                <div className="pt-4 flex justify-end gap-2">
                  ...
                  <button type="submit" className="bg-[#3730A3] text-white px-5 py-2 rounded-lg font-medium hover:bg-[#312E81] shadow-sm">Save Ad</button>
                </div>
              </form>
            </motion.div>
          </div>
