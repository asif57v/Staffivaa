...
export const createCategory = asyncHandler(async (req, res) => {
  const { groupId, name, subtitle = '', sortOrder = 0, imageUrl, baseRate = 800, platformFee = 0 } = req.body
  ...
  const c = await LabourCategory.create({
    ...
    platformFee: Number(platformFee) >= 0 ? Number(platformFee) : 0,
    sortOrder,
    isActive: true,
  })
...
export const patchCategory = asyncHandler(async (req, res) => {
  ...
  const { name, subtitle, sortOrder, isActive, imageUrl, baseRate, platformFee, groupId } = req.body
  ...
  if (platformFee != null) c.platformFee = Number(platformFee) >= 0 ? Number(platformFee) : 0
